...
random_init( random_t *t, long unsigned inseed)
{
  ...
  if (inseed < 1) {
    ...
    inseed = (long unsigned)abs(100*(long unsigned)time(NULL)+(long unsigned)clock());
  }
...
readpara_SupplementDefaults(readpara_t *t)
{
  ...
  if (t->seed < 1) {
    while ((int) (cloc - clock()) == 0)
      ; /* TODO: remove this for time critical applications!? */
    t->seed = (unsigned int)abs(100*(long unsigned)time(NULL)+(long unsigned)clock());
  }
